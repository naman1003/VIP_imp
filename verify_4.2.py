...
while(i<=256):
	...
	for j in range(100):
		...
		for k in range(i):
			x1.append(random.randint(-1*2**10 ,2**10))
			x2.append(random.randint(-1*2**10 ,2**10))
			x3.append(x_input[k]-x1[k]-x2[k])
			y1.append(random.randint(-1*2**10 ,2**10))
			y2.append(random.randint(-1*2**10 ,2**10))
			y3.append(x_input[k]-x1[k]-x2[k])
			x_server1.append([x1[k],x2[k]])
			x_server2.append([x2[k],x3[k]])
			x_server3.append([x3[k],x1[k]])
			y_server1.append([y1[k],y2[k]])
			y_server2.append([y2[k],y3[k]])
			y_server3.append([y3[k],y1[k]])

		c1_sharesofx =  c1_sharesofx + sys.getsizeof(x_server3) + sys.getsizeof(x_server2) + sys.getsizeof(x_server1)
		c2_sharesofy =  c2_sharesofy + sys.getsizeof(y_server3) + sys.getsizeof(y_server2) + sys.getsizeof(y_server1)		
	...
	seperate_data[0] = i
	seperate_data[1] = c1_sharesofx/100
	seperate_data[2] = c2_sharesofy/100
	seperate_data[3] = publish_tow/100
	seperate_data[4] = publish_T/100
	data.append(seperate_data)
